...
const Logos = ({ selectedTag, searchQuery }) => {
  const [logosItems, setLogosItems] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter only logos
        setLogosItems(res.data.filter(item => item.type.toLowerCase() === "logos"));
      } catch (err) {
