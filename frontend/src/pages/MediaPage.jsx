...
export default function MediaPage({ category, selectedTag }) {
  ...
  useEffect(() => {
    const fetchMedia = async () => {
      try {
        ...
        setMediaItems(res.data); // fetch all media from backend
      } catch (err) {
  ...
  const filteredItems = mediaItems.filter((item) => {
    const matchesCategory =
      category === "All" || normalize(item.type) === normalize(category);
    const matchesTag =
      selectedTag === "All" ||
      item.tags.some((tag) => normalize(tag) === normalize(selectedTag));
  ...
  return (
    <div className="container mx-auto px-4 py-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
      {filteredItems.map((item, index) => (
        <div key={item._id || index} className="bg-white rounded-lg shadow-lg overflow-hidden">
          {item.isVideo ? (
            <iframe
