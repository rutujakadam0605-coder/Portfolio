import React, { useState, useEffect } from "react";import axios from "axios";import Masonry from "react-masonry-css";

const breakpointColumnsObj = { default: 4, 1100: 3, 700: 2, 500: 1 };

// ✅ ENV-BASED APIconst API_BASE = import.meta.env.VITE_API_BASE_URL;

// Type options and tagsconst typeOptions = {Graphic: ["illustration", "poster", "flyer", "digital art"],Video: ["intro", "trailer", "ad", "animation"],UiDesign: ["wireframe", "mockup", "landingpage", "prototype"],Logos: ["minimal", "vintage", "mascot", "typography"],presentation: ["All", "Crypto", "Forex", "RealEstate", "Agriculture", "Corporate"],};

const AdminLayout = () => {const [media, setMedia] = useState([]);const [type, setType] = useState("Graphic");const [tags, setTags] = useState([]);const [file, setFile] = useState(null);const [externalUrl, setExternalUrl] = useState("");const [notification, setNotification] = useState("");

const showNotification = (msg) => {setNotification(msg);setTimeout(() => setNotification(""), 3000);};

/* ================= FETCH MEDIA ================= */const fetchMedia = async () => {try {const res = await axios.get(${API_BASE}/api/media);setMedia(res.data || []);} catch (err) {console.error(err);showNotification("❌ Failed to fetch media");}};

useEffect(() => {fetchMedia();}, []);

/* ================= MEDIA UPLOAD ================= */const handleUpload = async (e) => {e.preventDefault();
...
/* ================= DELETE MEDIA ================= */const handleDelete = async (id) => {try {await axios.delete(${API_BASE}/api/media/${id});setMedia((prev) => prev.filter((m) => m._id !== id));showNotification("🗑️ Deleted successfully");} catch (err) {console.error(err);showNotification("❌ Delete failed");}};

/* ================= BROCHURE UPLOAD ================= */const handleBrochureUpload = async (e) => {e.preventDefault();
